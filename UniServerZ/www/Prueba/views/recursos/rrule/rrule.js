...
(function (root, factory) {
  ...
  var dateutil = {
    ...
    getYearDay: function (date) {
      ...
      return Math.ceil(
        (dateNoTime - new Date(date.getFullYear(), 0, 1)) / dateutil.ONE_DAY
      ) + 1
    },
    ...
    getMonthDays: function (date) {
      ...
      return month === 1 && dateutil.isLeapYear(date) ? 29 : dateutil.MONTH_DAYS[
        month]
    },
    ...
    timeToUntilString: function (time) {
      ...
      for (var i = 0; i < comps.length; i++) {
        comp = comps[i]
        if (!/[TZ]/.test(comp) && comp < 10) comps[i] = '0' + String(
          comp)
      }
      return comps.join('')
    },
  ...
  var split = function (str, sep, num) {
    ...
    return num ? splits.slice(0, num).concat([splits.slice(num).join(sep)]) :
      splits
  }
  ...
  var plb = function (obj) {
    return (obj instanceof Array && obj.length === 0) ? false : Boolean(
      obj)
  }
  ...
  var M366RANGE = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335,
    366
  ]
  var M365RANGE = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334,
    365
  ]

  var WDAYMASK = (function () {
    for (var wdaymask = [], i = 0; i < 55; i++) wdaymask = wdaymask.concat(
      range(7))
    return wdaymask
  }())
  ...
  var RRule = function (options, noCache) {
    ...
    defaultKeys.forEach(function (key) {
      if (!contains(keys, key)) options[key] = RRule.DEFAULT_OPTIONS[
        key]
    })
  ...
  RRule.optionsToString = function (options) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      default:
        if (value instanceof Array) {
          for (j = 0; j < value.length; j++) strValues[j] = String(value[
            j])
          value = strValues
        } else {
          value = String(value)
  ...
  RRule.prototype = {
    ...
    between: function (after, before, inc, iterator) {
      ...
      if (iterator) {
        return this._iter(new CallbackIterResult('between', args,
          iterator))
      }
    ...
    _cacheAdd: function (what, value, args) {
      ...
      if (value) {
        value = (value instanceof Date) ? dateutil.clone(value) :
    ...
    _cacheGet: function (what, args) {
      ...
      return cached instanceof Array ? dateutil.cloneDates(cached) : (
    ...
    _iter: function (iterResult) {
      ...
      */
      ...
      if (freq < RRule.HOURLY) {
        ...
        if ((freq >= RRule.HOURLY && plb(byhour) && !contains(byhour,
            hour)) ||
          (freq >= RRule.MINUTELY && plb(byminute) && !contains(
            byminute, minute)) ||
          (freq >= RRule.SECONDLY && plb(bysecond) && !contains(
            bysecond, minute))) {
          timeset = []
        } else {
  ...
  Iterinfo.prototype.rebuild = function (year, month) {
    ...
    if (year !== this.lastyear) {
      ...
      if (!plb(rr.options.byweekno)) {
        ...
        no1wkst = firstwkst = pymod(7 - this.yearweekday + rr.options.wkst,
          7)
        if (no1wkst >= 4) {
          no1wkst = 0
            // Number of days in the year, plus the days we got
            // from last year.
          wyearlen = this.yearlen + pymod(this.yearweekday - rr.options.wkst,
            7)
        } else {
        ...
        if (no1wkst) {
          ...
          var lnumweeks
          if (!contains(rr.options.byweekno, -1)) {
            var lyearweekday = dateutil.getWeekday(new Date(year - 1, 0,
              1))
            ...
            if (lno1wkst >= 4) {
              ...
              lnumweeks = Math.floor(52 +
                pymod(lyearlen + pymod(lyearweekday - rr.options.wkst,
                  7), 7) / 4)
            } else {
              lnumweeks = Math.floor(52 + pymod(this.yearlen - no1wkst, 7) /
                4)
            }
          } else {
            lnumweeks = -1
    ...
    if (plb(rr.options.bynweekday) && (month !== this.lastmonth || year !==
        this.lastyear)) {
      var ranges = []
  ...
  IterResult.prototype = {
    ...
    init: function (method, args) {
      ...
      if (method === 'between') {
        this.maxDate = args.inc ? args.before : new Date(args.before.getTime() -
          1)
        this.minDate = args.inc ? args.after : new Date(args.after.getTime() +
          1)
      } else if (method === 'before') {
        this.maxDate = args.inc ? args.dt : new Date(args.dt.getTime() -
          1)
      } else if (method === 'after') {
        this.minDate = args.inc ? args.dt : new Date(args.dt.getTime() +
          1)
      }
    },
   ...
   */
  var RRuleSetMethods = ['all', 'between', 'before', 'after', 'count',
    '_cacheAdd', '_cacheGet'
  ]
  ...
  RRuleStr.prototype = {
    ...
    _parseRfc: function (s, options) {
      ...
      var exrulevals = []
      var exdatevals = []
      var name, value, parts, parms, parm, dtstart, rset, j, k,
        datestrs, datestr

      if (!options.forceset && lines.length === 1 && (s.indexOf(':') ===
          -1 ||
          s.indexOf('RRULE:') === 0)) {
        ...
        if (options.forceset || rrulevals.length > 1 || rdatevals.length ||
          ...
          if (options.campatiable && options.dtstart) rset.rdate(
            dtstart)
          return rset
        } else {
    ...
    parse: function (s, options) {
      ...
      if (invalid.length) throw new Error('Invalid options: ' + invalid
        .join(', '))

      // Merge in default options
      defaultKeys.forEach(function (key) {
        if (!contains(keys, key)) options[key] = RRuleStr.DEFAULT_OPTIONS[
          key]
      })
  ...
  RRuleStr.prototype._handle_DTSTART = function (rrkwargs, name, value,
    options) {
