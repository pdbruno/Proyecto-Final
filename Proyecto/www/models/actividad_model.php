...
class actividad_Model extends Model {
  ...
  {
    for ($i = 0; $i < count($data); $i++) {
      $this->db->query("INSERT INTO `asistencias` SET `idClientes`= ?i, `idActividades`= ?i, `Fecha`= ?s, idSubactividades = (SELECT idSubactividades FROM `subactividades` WHERE `idEvento` = ?s)", $data[$i], $idActividades, $fecha, $idEvento);
    }
  }
